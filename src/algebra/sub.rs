...
mod tests {
    ...
    fn test() {
        ...
        assert_eq!((x.clone() - y.clone()).gradient("x").eval(), ndarray::arr1(&[1.0, 1.0, 1.0]).into_dyn());
        assert_eq!((x.clone() - y.clone()).gradient("y").eval(), ndarray::arr1(&[-1.0, -1.0, -1.0]).into_dyn());
    }
}
