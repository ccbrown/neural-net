...
impl ExprImpl for Mul {
    ...
    fn accumulate_gradients(
        ...
        vec![
            Some(output.clone() * self.right.clone()),
            Some(output.clone() * self.left.clone()),
        ]
    }
...
mod tests {
    ...
    fn test() {
        ...
        assert_eq!(
            (3.0 * x.clone()).gradient("x").eval(),
            ndarray::arr0(3.0).into_dyn()
        );
        ...
        assert_eq!(
            ...
            ndarray::arr1(&[0.0, 1.0, 5.0]).into_dyn()
        );
    }
}
